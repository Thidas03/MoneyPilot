...
const TransactionModal = ({ isOpen, onClose, onSubmit, transaction = null }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-dark-950/60 backdrop-blur-sm animate-fade-in">
      {/* Modal Dialog Body */}
      <div className="relative w-full max-w-lg glass-panel rounded-3xl p-6 sm:p-8 shadow-2xl overflow-hidden animate-slide-up">
        {/* Glow corner */}
        <div className="absolute -top-10 -right-10 w-24 h-24 bg-brand-500/10 rounded-full blur-2xl"></div>
        ...
        <div className="flex items-center justify-between pb-4 border-b border-white/5">
          ...
          <button
            onClick={onClose}
            className="text-dark-400 hover:text-white p-1 rounded-lg transition-colors focus:outline-none"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-5">
            ...
            <button
              ...
              className={`py-3 px-4 rounded-xl text-sm font-semibold tracking-wide border transition-all ${
                formData.type === 'expense'
                  ...
                  : 'bg-white/5 border-white/5 text-dark-300 hover:bg-white/10'
              }`}
            >
              Expense
            </button>
          ...
          <div className="flex justify-end gap-4 pt-4 border-t border-white/5">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 rounded-xl bg-white/5 border border-white/5 text-dark-200 text-sm font-semibold hover:bg-white/10 active:scale-[0.98] transition-all"
            >
              Cancel
            </button>
            <button
